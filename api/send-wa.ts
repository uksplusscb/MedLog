...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const sendWithToken = async (activeToken: string) => {
    ...
    try {
      ...
      console.log(`[WA Vercel] Mengirim ke ${target} menggunakan token: ${maskedToken} pada ${new Date().toISOString()}`);
      ...
      try {
        ...
        console.warn(`[WA Vercel] FormData approach failed (${formDataErr.message || formDataErr}), trying URLSearchParams fallback...`);
      ...
      console.log(`[WA Vercel] Respons Mentah Fonnte (${usedMethod}) untuk ${target} dengan Status HTTP ${response.status}: ${resText.substring(0, 500)}`);

      let resData: any = {};
      try {
        resData = JSON.parse(resText);
      } catch (parseErr) {
        console.warn(`[WA Vercel] Gagal mengubah respons Fonnte ke JSON. Menggunakan fallback objek.`);
      ...
      clearTimeout(timeoutId);
      console.error(`[WA Vercel] Error Fonnte ke ${target}:`, error);
      return {
        success: false,
        status: 500,
        data: {
  ...
  try {
    ...
    if (!result.success && token !== defaultToken) {
      console.warn(`[WA Vercel] Token kustom gagal (${result.data.reason || result.data.detail || "disconnected/unreachable"}). Mencoba otomatis menggunakan Token Default UKS...`);
      const backupResult = await sendWithToken(defaultToken);
      if (backupResult.success) {
        console.log(`[WA Vercel] Berhasil mengirim pesan via Token Default UKS (Fallback sukses)`);
        result = {
          success: true,
          status: 200,
          data: {
        ...
        console.error(`[WA Vercel] Token kustom AND Token default gagal terkirim.`);
      }
    }

    return res.status(result.status).json(result.data);
  } catch (error: any) {
    console.error("Fonnte API handler error:", error);
    return res.status(500).json({ 
      status: false, 
